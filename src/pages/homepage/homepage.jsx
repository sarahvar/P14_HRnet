    ...
    const {
        ...
        validations: {
            firstName: {
                ...
                pattern: { value: nameRegex, message: "First Name must contain only letters, spaces and some special characters" },
            },
            lastName: {
                required: { value: true, message: "Last Name is required" },
                pattern: { value: nameRegex, message: "Last Name must contain only letters, spaces and some special characters" },
            },
            dateOfBirth: {
                required: { value: true, message: "Date of Birth is required" },
                pattern: { value: dateRegex, message: "Date of Birth isn't in the format MM/DD/YYYY" },
            },
            startDate: {
                required: { value: true, message: "Start Date is required" },
                pattern: { value: dateRegex, message: "Start Date isn't in the format MM/DD/YYYY" },
            },
            street: {
                required: { value: true, message: "Street is required" },
                pattern: { value: addressRegex, message: "Street must contain only letters, numbers, spaces and some special characters" },
            },
            city: {
                required: { value: true, message: "City is required" },
                pattern: { value: addressRegex, message: "City must contain only letters, numbers, spaces and some special characters" },
            },
            state: {
                required: { value: true, message: "State is required" },
                custom: { isValid: (value) => states.some((state) => state.value === value), message: "Your selection is not valid" },
            },
            zip: {
                required: { value: true, message: "Zip is required" },
                pattern: { value: zipRegex, message: "Zip must be 4 or 5 digits" },
            },
            department: {
                required: { value: true, message: "Department is required" },
                custom: { isValid: (value) => departments.includes(value), message: "Your selection is not valid" },
            },
        },
        onSubmit: () => {
            const employee = {
                ...
                >
                    ...
                    <fieldset className="address">
                        ...
                        <div className="form-group">
                            <Dropdown id="state" label="State" value={data.state} options={states} onChange={handleChange("state")} listLabel="Choose your state" showListLabel={true} requiredFeedbackEnabled={true} />
                        </div>
                        <Input
                            id="zip"
                            label="Zip Code"
